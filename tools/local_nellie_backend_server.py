            ...
            self._send(
                {
                    ...
                    "capabilities": ["runtime_model_switching", "response_feedback"],
                }
            )
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.path == "/v1/feedback/save":
                ...
                self._send({"ok": True, "id": feedback_id})
                return
...
def main() -> None:
    ...
    print(f"Nellie backend listening on http://{host}:{port}", flush=True)
    try:
