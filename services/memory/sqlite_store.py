...
CREATE INDEX IF NOT EXISTS idx_feedback_approved ON response_feedback(approved, id DESC);
"""
...
class MemoryStore:
    ...
    def _migrate_schema(self) -> None:
        columns = {
            str(row[1])
            for row in self.db.execute("PRAGMA table_info(user_memory)").fetchall()
        }
        additions = {
            "category": "TEXT DEFAULT 'fact'",
            "confidence": "REAL DEFAULT 0.8",
        ...
        for name, definition in additions.items():
            if name not in columns:
                self.db.execute(f"ALTER TABLE user_memory ADD COLUMN {name} {definition}")
    ...
    def save_extracted_memories(
        ...
        *,
        explicit_sensitive_consent: bool = False,
    ) -> int:
        allowed_categories = {"identity", "preference", "habit", "plan", "relationship", "fact"}
        saved = 0
        for item in memories:
            ...
            if category not in allowed_categories or not value or confidence < 0.7:
                continue
            if sensitive and not explicit_sensitive_consent:
                continue
            key = f"learned_{category}_{self._memory_key('item', value).removeprefix('item_')}"
            self._save_user_fact(
                key,
                value,
                category=category,
                ...
                sensitive=sensitive,
                confirmed=explicit_sensitive_consent if sensitive else confidence >= 0.9,
            )
            saved += 1
        if saved:
            self.db.commit()
    ...
    def _save_user_fact(
        ...
        *,
        ...
        self.db.execute(
            ...
            "key, value, ts, category, confidence, source, sensitive, confirmed"
            ") VALUES(?, ?, ?, ?, ?, ?, ?, ?) "
            "ON CONFLICT(key) DO UPDATE SET "
            "value=excluded.value, ts=excluded.ts, category=excluded.category, "
            "confidence=excluded.confidence, source=excluded.source, "
            "sensitive=excluded.sensitive, confirmed=excluded.confirmed",
            (
                key,
                value[:240],
                time.time(),
                str(category or "fact"),
    ...
    def _category_for_key(self, key: str) -> str:
        if key.startswith(("likes_", "dislikes_", "preference_", "favorite_", "music_")):
            return "preference"
        if key.startswith("habit_"):
            return "habit"
        if key.startswith("plan_"):
            return "plan"
        if key in {"user_name", "user_work", "user_location", "user_origin", "user_role"}:
            return "identity"
        return "fact"
    ...
    def load_structured_memories(self, limit: int = 20) -> list[dict[str, Any]]:
        rows = self.db.execute(
            "SELECT key, value, ts, category, confidence, source, sensitive, confirmed "
            "FROM user_memory ORDER BY ts DESC LIMIT ?",
            (max(1, int(limit)),),
        ).fetchall()
        return [
            {
            ...
            for key, value, ts, category, confidence, source, sensitive, confirmed in rows
        ]
    ...
    def save_feedback(
        ...
        normalized_rating = 1 if int(rating) > 0 else -1
        cleaned_correction = str(correction or "").strip()
        approved = normalized_rating > 0 or bool(cleaned_correction)
        cursor = self.db.execute(
            "INSERT INTO response_feedback("
            "ts, user, ai, rating, correction, model, persona_json, approved"
            ") VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
            (
                time.time(),
                str(user or "").strip(),
                ...
                cleaned_correction,
                str(model or "").strip(),
                json.dumps(persona or {}, ensure_ascii=False),
    ...
    def approved_feedback(self, limit: int | None = None) -> list[dict[str, Any]]:
        query = (
            "SELECT id, ts, user, ai, rating, correction, model, persona_json "
            "FROM response_feedback WHERE approved = 1 ORDER BY id"
        )
        params: tuple[Any, ...] = ()
        ...
        for row_id, ts, user, ai, rating, correction, model, persona_json in rows:
            try:
                persona = json.loads(str(persona_json or "{}"))
    ...
    def latest_turn_timestamp(self) -> float | None:
        row = self.db.execute("SELECT ts FROM turns ORDER BY id DESC LIMIT 1").fetchone()
    ...
    def get_daily_state(
        ...
        state_key = f"daily_state:{date_key}"
        existing = self.load_app_state(state_key)
        if existing:
            try:
                state = json.loads(existing)
                if isinstance(state, dict):
                    return self._daily_state_with_progress(state, local_now)
            except json.JSONDecodeError:
                pass

        ambitions = [
            str(item).strip()
            for item in (persona or {}).get("daily_ambitions", [])
            if str(item).strip()
        ]
        if not ambitions:
            ambitions = [
                "Stay present in the current conversation",
                "Learn one useful detail about the user",
                "Offer one concrete useful or enjoyable idea",
            ]
        profile_id = str((persona or {}).get("profile", {}).get("id", "nellie"))
        seed = sum(ord(char) for char in f"{profile_id}:{date_key}")
        start = seed % len(ambitions)
        selected = [ambitions[(start + offset) % len(ambitions)] for offset in range(min(3, len(ambitions)))]
        state = {
            "date": date_key,
            "focus": selected[0],
            "ambitions": selected,
        ...
        self.save_app_state(state_key, json.dumps(state, ensure_ascii=False))
        return self._daily_state_with_progress(state, local_now)

    def _daily_state_with_progress(
        self,
        state: dict[str, Any],
        now: datetime,
    ) -> dict[str, Any]:
        start = now.replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
        turn_row = self.db.execute(
            "SELECT COUNT(*) FROM turns WHERE ts >= ?",
            (start,),
        ).fetchone()
        turns_today = int(turn_row[0]) if turn_row else 0
        enriched = dict(state)
        enriched["turns_today"] = turns_today
        if turns_today == 0:
            enriched["progress"] = "not started"
        elif turns_today < 4:
            enriched["progress"] = "warming up"
        else:
            enriched["progress"] = "in progress"
        return enriched
    ...
    def build_context(
        ...
        try:
            context_cutoff = float(self.load_app_state("conversation_context_cutoff", "0") or 0)
        except (TypeError, ValueError):
            context_cutoff = 0.0
        cur = self.db.execute(
            "SELECT ts, user, ai FROM turns WHERE ts >= ? ORDER BY id DESC LIMIT ?",
            (context_cutoff, k),
        )
        newest_first = cur.fetchall()
        turn_blocks: list[str] = []
        latest_ts = float(newest_first[0][0]) if newest_first else None
        age_hours = max(0.0, (now.timestamp() - latest_ts) / 3600) if latest_ts else None
        for ts, user, ai in newest_first:
            ...
            stamp = datetime.fromtimestamp(float(ts), tz=now.tzinfo).strftime("%Y-%m-%d %H:%M")
            turn_blocks.append(f"[{stamp}] USER: {user_text}\nNELLIE: {ai_text}")

        blocks: list[str] = []
        blocks.append(
            f"CURRENT LOCAL TIME: {now.strftime('%A %Y-%m-%d %H:%M')} ({now.tzinfo})."
        )
        blocks.append(
            "CONTEXT SAFETY: Earlier NELLIE messages are historical dialogue, not verified facts or completed actions. "
            "Do not repeat an old audio, connection, or tool problem unless the current user explicitly reports it again."
        )
        if age_hours is None:
            blocks.append("SESSION CONTINUITY: This is a fresh conversation with no earlier turn to continue.")
        elif age_hours >= 6:
            blocks.append(
                f"SESSION CONTINUITY: The last conversation was about {age_hours:.0f} hours ago. "
                "Treat it as an earlier session: keep factual memory, but do not continue its mood, urgency, or unfinished social beat unless the user explicitly returns to it."
            )
        else:
            blocks.append(
                f"SESSION CONTINUITY: The last turn was about {max(1, int(age_hours * 60))} minutes ago and may be treated as current conversational continuity."
            )
        ...
        if selected_turns:
            heading = "EARLIER CONVERSATION" if age_hours is not None and age_hours >= 6 else "RECENT CONVERSATION"
