...
def sanitize_assistant_reply(text: str) -> str:
    ...
    if not cleaned:
        return ""
    cleaned = re.sub(r"\*[^*\n]{1,120}\*", "", cleaned)
    cleaned = re.sub(r"(?:^|\s)\*[A-Za-z]{1,18}\s*$", "", cleaned)
...
def prepare_tts_text(text: str, mood: str, tts_conf: dict[str, Any], persona: dict[str, Any]) -> str:
    ...
    if not text:
        return ""
    text = re.sub(r"\bmm+\b", lambda match: _normalized_interjection(match.group(0), "mm"), text, flags=re.IGNORECASE)
    text = re.sub(r"\boh+\b", lambda match: _normalized_interjection(match.group(0), "oh"), text, flags=re.IGNORECASE)
    text = re.sub(r"\bah+\b", lambda match: _normalized_interjection(match.group(0), "ah"), text, flags=re.IGNORECASE)
    text = re.sub(r"\.\.\.+", ", ", text)
...
def _normalized_interjection(original: str, normalized: str) -> str:
    return normalized.capitalize() if original[:1].isupper() else normalized
