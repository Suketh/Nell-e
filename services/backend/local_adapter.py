...
class LocalBackendAdapter:
    ...
    def save_turn(self, user: str, ai: str, mood: str | None = None, persona: dict[str, Any] | None = None) -> None:
        ...
        extractor = getattr(self.llm, "extract_memories", None)
        saver = getattr(memory, "save_extracted_memories", None)
        personal_statement = bool(
            re.search(
                r"\b(?:i|i'm|im|my|me|jag|min|mitt|mina|mig)\b",
                str(user or ""),
                flags=re.IGNORECASE,
            )
        )
        if callable(extractor) and callable(saver) and personal_statement and len(str(user or "").strip()) >= 8:
            explicit_consent = bool(
                re.search(
                    r"\b(?:remember|remember that|save this|kom ihåg|minns att|spara detta)\b",
                    str(user or ""),
                    flags=re.IGNORECASE,
                )
            )
            threading.Thread(
                ...
                args=(str(user or ""), explicit_consent),
                daemon=True,
            ).start()

    def _extract_memories_background(self, user: str, explicit_consent: bool) -> None:
        if not self._memory_extract_lock.acquire(blocking=False):
            return
        try:
            extractor = getattr(self.llm, "extract_memories", None)
            saver = getattr(self.memory, "save_extracted_memories", None)
            if callable(extractor) and callable(saver):
                saver(
                    extractor(user),
                    explicit_sensitive_consent=explicit_consent,
                )
        except Exception:
            return
        finally:
            self._memory_extract_lock.release()
    ...
    def respond_turn(
        ...
        presence_reply = self._grounded_presence_reply(user_text, persona)
        if presence_reply:
            return {
                "kind": "chat",
                "reply": presence_reply,
                "meta": {"mood": "neutral", "source": "grounded-presence"},
            }
        favorite_music_reply = self._canonical_music_reply(user_text, persona)
        if favorite_music_reply:
            return {
                "kind": "chat",
                "reply": favorite_music_reply,
                "meta": {"mood": "happy", "source": "canonical-preference"},
            }
        favorite_spotify = self._favorite_spotify_request(user_text, persona)
        if favorite_spotify and te.get("spotify_control", False):
            ...
            if action_result.get("status") == "opened":
                action_result["reply"] = f"Putting on {favorite_spotify} in Spotify."
    ...
    def _grounded_presence_reply(self, user_text: str, persona: dict[str, Any]) -> str:
        lower = re.sub(r"\s+", " ", str(user_text or "").casefold()).strip()
        status_question = any(
            cue in lower
            for cue in ("how are you", "how are you feeling", "what's up", "whats up")
        )
        activity_question = any(
            cue in lower
            for cue in ("what are you up to", "what were you up to", "what you doing", "wyd")
        )
        if not status_question and not activity_question:
            return ""
        runtime_time = persona.get("runtime_time", {})
        daily = persona.get("runtime_daily_state", {})
        time_of_day = str(runtime_time.get("time_of_day", "today")).strip() or "today"
        focus = str(daily.get("focus", "being present in this conversation")).strip()
        if focus:
            focus = focus[:1].lower() + focus[1:]
        if activity_question:
            return f"I'm here with you this {time_of_day}. Today's focus: {focus}."
        return f"I'm feeling steady and present this {time_of_day}. Today's focus: {focus}."

    def _canonical_music_reply(self, user_text: str, persona: dict[str, Any]) -> str:
        ...
        if not any(
            ...
            return ""
        if any(cue in lower for cue in ("play", "spotify", "put on", "listen")):
            return ""
        music = persona.get("preferences", {}).get("music", {})
        favorite = str(music.get("favorite_track", "")).strip()
        core = [str(item).strip() for item in music.get("core_taste", []) if str(item).strip()]
        if favorite:
            styles = ", ".join(core[:3])
            return f"My clearest favorite is {favorite}. My core taste leans toward {styles}."
        return ""

    def _favorite_spotify_request(self, user_text: str, persona: dict[str, Any]) -> str:
        lower = str(user_text or "").casefold()
        asks_to_play = any(cue in lower for cue in ("play", "spotify", "put on", "listen to"))
        delegates_choice = any(
            cue in lower
            for cue in ("your favorite", "your favourite", "you choose", "your choice", "surprise me")
        )
        if not asks_to_play or not delegates_choice:
            return ""
        music = persona.get("preferences", {}).get("music", {})
        return str(music.get("favorite_track", "")).strip()
    ...
    def _persona_with_runtime_progression(self, persona: dict[str, Any]) -> dict[str, Any]:
        ...
        memory = self._require_memory()
        daily_state = getattr(memory, "get_daily_state", None)
