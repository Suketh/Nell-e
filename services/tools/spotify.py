...
def extract_spotify_query(text: str, context: str = "") -> str:
    ...
    explicit_contextual_patterns = [
        ...
        r"^(?:why don'?t you )?(?:start|start up|open) (?:it up )?on spotify(?: for me)?[.!? ]*$",
        r"^(?:why don'?t you )?(?:start|start up|open) spotify(?: for me)?[.!? ]*$",
    ]
    ...
    patterns = [
        r"^(?:can you |could you |please )?(?:start|start up|open) spotify (?:with|and play) (.+?)[.!? ]*$",
...
def _resolve_contextual_query(context: str) -> str:
    ...
    suggested_track = re.findall(
        r'\bhow about\s+["“]?(.{2,100}?)["”]?\s+by\s+'
        r"([A-Za-z][\w'&.-]*(?:\s+[A-Za-z][\w'&.-]*){0,4})(?=[.!?\n]|$)",
        source,
        flags=re.IGNORECASE,
    )
    if suggested_track:
        title, artist = suggested_track[-1]
        cleaned_title = title.strip(' ,.!?"“”')
        return f"{cleaned_title} {artist.strip(' ,.!?')}"
...
def _has_spotify_intent(context: str) -> bool:
    ...
    pending_music_action = any(
        ...
        for cue in (
            "play me a song",
            "play me one of your favorite tunes",
            "play me one of your favourite tunes",
            "favorite tune",
            "favourite tune",
            "play something",
            "i'll play",
