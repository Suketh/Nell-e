...
from services.backend.speech_prep import sanitize_assistant_reply
...
class SettingsDialog(QDialog):
    def __init__(self, parent: "MainWindow") -> None:
        ...
        voice_label = QLabel("Application audio")
        ...
        speaker_label.setObjectName("controlLabel")
        voice_layout.addWidget(speaker_label)

        self.audio_output_select = QComboBox()
        self.audio_output_select.setObjectName("voiceProfile")
        self.audio_output_select.currentIndexChanged.connect(self._on_audio_output_changed)
        voice_layout.addWidget(self.audio_output_select)
        ...
        self.microphone_mute_toggle = QCheckBox("Mute microphone")
        self.microphone_mute_toggle.toggled.connect(self.window.set_microphone_muted)
        speech_layout.addWidget(self.microphone_mute_toggle)

        self.refresh_audio_btn = QPushButton("Refresh Audio Devices")
        self.refresh_audio_btn.setObjectName("secondaryButton")
        self.refresh_audio_btn.clicked.connect(self.window.refresh_audio_devices)
        speech_layout.addWidget(self.refresh_audio_btn)
    ...
    def sync_from_window(self) -> None:
        ...
        self.audio_output_select.blockSignals(True)
        self.microphone_mute_toggle.blockSignals(True)
        ...
        self.voice_toggle.setText("Mute Nellie" if enabled else "Unmute Nellie")
        ...
        self.audio_output_select.clear()
        for device_id, label in self.window.available_audio_outputs:
            self.audio_output_select.addItem(label, device_id)
        output_index = self.audio_output_select.findData(self.window.current_audio_output_device)
        if output_index >= 0:
            self.audio_output_select.setCurrentIndex(output_index)
        elif self.audio_output_select.count() > 0:
            self.audio_output_select.setCurrentIndex(0)
        self.microphone_mute_toggle.setChecked(self.window.microphone_muted)
        ...
        self.audio_output_select.blockSignals(False)
        self.microphone_mute_toggle.blockSignals(False)
    ...
    def _on_audio_output_changed(self) -> None:
        device = self.audio_output_select.currentData()
        self.window.set_audio_output_device(device)
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.available_audio_inputs, self.available_audio_outputs = self._detect_audio_devices()
        ...
        configured_audio_input = audio_conf.get("input_device")
        stored_audio_input = state_store.load_app_state("audio_input_device")
        if stored_audio_input is None and configured_audio_input not in {None, ""}:
            stored_audio_input = str(configured_audio_input)
        self.current_audio_input_device = None if stored_audio_input in {None, "", "__default__"} else stored_audio_input
        available_input_ids = {device_id for device_id, _ in self.available_audio_inputs}
        if self.current_audio_input_device not in available_input_ids:
            self.current_audio_input_device = None
            state_store.save_app_state("audio_input_device", "__default__")
        ...
        configured_audio_output = audio_conf.get("output_device")
        stored_audio_output = state_store.load_app_state("audio_output_device")
        if stored_audio_output is None and configured_audio_output not in {None, ""}:
            stored_audio_output = str(configured_audio_output)
        self.current_audio_output_device = (
            None if stored_audio_output in {None, "", "__default__"} else stored_audio_output
        )
        available_output_ids = {device_id for device_id, _ in self.available_audio_outputs}
        if self.current_audio_output_device not in available_output_ids:
            self.current_audio_output_device = None
            state_store.save_app_state("audio_output_device", "__default__")
        self._apply_audio_output_device()
        self.microphone_muted = state_store.load_app_state("microphone_muted", "0") == "1"
        tts_enabled = state_store.load_app_state("tts_enabled", "1") == "1"
        tts_volume = int(state_store.load_app_state("tts_master_volume", "100") or 100)
        self.tts.set_enabled(tts_enabled)
        self.tts.set_master_volume(tts_volume)
        ...
        self.recorder.set_muted(self.microphone_muted)
        ...
        self.chat.feedback_requested.connect(self._save_response_feedback)
    ...
    def _detect_audio_devices(self) -> tuple[list[tuple[str | None, str]], list[tuple[str | None, str]]]:
        inputs: list[tuple[str | None, str]] = [(None, "System Default")]
        outputs: list[tuple[str | None, str]] = [(None, "System Default")]
        try:
            import sounddevice as sd  # type: ignore

            devices = sd.query_devices()
            for index, device in enumerate(devices):
                name = str(device.get("name", f"Audio device {index}")).strip() or f"Audio device {index}"
                if int(device.get("max_input_channels", 0) or 0) > 0:
                    inputs.append((str(index), f"{name} [{index}]"))
                if int(device.get("max_output_channels", 0) or 0) > 0:
                    outputs.append((str(index), f"{name} [{index}]"))
        except Exception:
            pass
    ...
    def _active_text_model(self) -> str:
        if self.ollama is not None and hasattr(self.ollama, "text_model"):
            return str(self.ollama.text_model)
        stored = self._load_state("ollama_text_model")
        if stored:
            return str(stored)
        return str(self.conf.get("ollama", {}).get("text_model", ""))

    def _save_response_feedback(self, user_text: str, ai_text: str, rating: int) -> None:
        correction = ""
        if rating < 0:
            correction, accepted = QInputDialog.getMultiLineText(
                self,
                "Improve Nellie's reply",
                "Write the response Nellie should have given. Leave it empty to only mark the reply as unhelpful.",
                ai_text,
            )
            if not accepted:
                return
        ...
        try:
            feedback_id = self._state_store().save_feedback(
                user=user_text,
                ai=ai_text,
                rating=rating,
                correction=correction,
                model=self._active_text_model(),
                persona=self.persona,
            )
        except Exception as exc:
            QMessageBox.warning(self, "Feedback not saved", str(exc))
            return
        status = "Saved as approved training feedback." if rating > 0 or correction else "Marked as unhelpful."
        self._record_admin_event("response_feedback", feedback_id=feedback_id, rating=rating)
    ...
    def _runtime_persona(self) -> dict[str, Any]:
        enriched = deepcopy(self.persona)
        enriched["runtime_progression"] = self._load_progression_state()
        enriched["runtime_time"] = lookup_local_datetime()
        daily_state = getattr(self._state_store(), "get_daily_state", None)
        if callable(daily_state):
            enriched["runtime_daily_state"] = daily_state(enriched)
        return enriched
    ...
    def set_tts_enabled(self, enabled: bool) -> None:
        ...
        self._save_state("tts_enabled", "1" if enabled else "0")
    ...
    def change_volume(self, value: int) -> None:
        ...
        self._save_state("tts_master_volume", str(int(value)))
        if hasattr(self, "settings_dialog"):
            self.settings_dialog.sync_from_window()
    ...
    def _system_default_audio_device(self, kind: str) -> int | None:
        try:
            import sounddevice as sd  # type: ignore

            host_api = sd.query_hostapis(sd.default.hostapi)
            key = "default_input_device" if kind == "input" else "default_output_device"
            value = int(host_api.get(key, -1))
    ...
    def _apply_audio_output_device(self) -> None:
        try:
            ...
            output_device = self.current_audio_output_device
            if output_device is None:
                resolved_output = self._system_default_audio_device("output")
            else:
                resolved_output = int(output_device)
            if resolved_output is None:
                return
            device_info = sd.query_devices(resolved_output)
            if int(device_info.get("max_output_channels", 0) or 0) <= 0:
                raise RuntimeError(f"Device {resolved_output} is not an audio output.")
            input_device = self._system_default_audio_device("input")
            sd.default.device = (input_device, resolved_output)
            self.conf.setdefault("audio", {})["output_device"] = (
                None if output_device is None else resolved_output
            )
        except Exception as exc:
            raise RuntimeError(f"Could not select audio output: {exc}") from exc

    def set_audio_output_device(self, device: Any) -> None:
        normalized = None if device in {None, "", "__default__"} else str(device)
        previous = self.current_audio_output_device
        self.current_audio_output_device = normalized
        try:
            self._apply_audio_output_device()
        except Exception as exc:
            self.current_audio_output_device = previous
            self._apply_audio_output_device()
            self.ai_message.emit(f"[Speakers unavailable] {exc}")
        else:
            self._save_state("audio_output_device", normalized or "__default__")
            label = next(
                (name for key, name in self.available_audio_outputs if key == normalized),
                "System Default",
            )
            self.ai_message.emit(f"[Speakers] Output device set to {label}.")
        if hasattr(self, "settings_dialog"):
            self.settings_dialog.sync_from_window()

    def set_microphone_muted(self, muted: bool) -> None:
        self.microphone_muted = bool(muted)
        self._save_state("microphone_muted", "1" if self.microphone_muted else "0")
        if hasattr(self, "recorder"):
            self.recorder.set_muted(self.microphone_muted)
        self.ai_message.emit(
            "[Microphone] Muted." if self.microphone_muted else "[Microphone] Ready."
        )
        if hasattr(self, "settings_dialog"):
            self.settings_dialog.sync_from_window()
    ...
    def refresh_audio_devices(self) -> None:
        self.available_audio_inputs, self.available_audio_outputs = self._detect_audio_devices()
        input_ids = {device_id for device_id, _ in self.available_audio_inputs}
        output_ids = {device_id for device_id, _ in self.available_audio_outputs}
        if self.current_audio_input_device not in input_ids:
            self.set_audio_input_device(None)
        if self.current_audio_output_device not in output_ids:
            self.set_audio_output_device(None)
        if hasattr(self, "settings_dialog"):
            self.settings_dialog.sync_from_window()
        self.ai_message.emit("[Audio] Device lists refreshed.")
    ...
    def _run_chat(self, reply_id: int, text: str, context: str, source: str = "text") -> None:
        ...
        try:
            ...
            runtime_persona = self.persona if self.backend is not None else self._runtime_persona()
    ...
    def _clean_ai_reply(self, reply: str) -> str:
        text = sanitize_assistant_reply(reply)
        if not text:
            return ""
    ...
    def _publish_reply(self, reply_id: int, user_text: str, reply: str, mood: str) -> None:
        ...
        allow_feedback = bool(reply.strip()) and not reply.lstrip().startswith("[")
        self.chat.add_ai(reply, user_text=user_text, allow_feedback=allow_feedback)
        self._turn_count += 1
        ...
        while True:
            ...
            try:
                ...
                self._record_admin_event(
                    ...
                    text=text,
                    mood=mood,
                    engine=str(getattr(self.tts, "last_engine", self.current_tts_engine)),
                    fallback_reason=str(getattr(self.tts, "last_fallback_reason", "") or ""),
                )
                if not self._is_sentence_chunk_tts():
                    engine_label = self.available_tts_engines.get(self.current_tts_engine, "Voice")
    ...
    def _prepare_tts_text(self, text: str, mood: str | None = None) -> str:
        ...
        text = re.sub(r"\bmm+\b", lambda match: "Mm" if match.group(0)[:1].isupper() else "mm", text, flags=re.IGNORECASE)
        text = re.sub(r"\boh+\b", lambda match: "Oh" if match.group(0)[:1].isupper() else "oh", text, flags=re.IGNORECASE)
        text = re.sub(r"\bah+\b", lambda match: "Ah" if match.group(0)[:1].isupper() else "ah", text, flags=re.IGNORECASE)
        text = re.sub(r"\.\.\.+", ", ", text)
